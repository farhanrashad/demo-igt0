...
class PurchaseBudgetReport(models.Model):
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
         count(*) as nbr, min(bl.id) as id, b.department_id, bl.expense_category, bl.name, bl.date_from, bl.date_to, bl.analytic_account_id, sum(bl.planned_quantity) as planned_quantity, avg(bl.planned_price_unit) as planned_price_unit, sum(bl.planned_quantity * bl.planned_price_unit) as planned_amount, sum(pl.product_qty) as actual_quantity, avg(pl.price_unit) as average_price, sum(pl.product_qty * pl.price_unit) as actual_amount, bl.purchase_budget_state,
         sum(bl.planned_quantity) - sum(pl.product_qty) as remain_qty, 
...
group by b.department_id, bl.expense_category, bl.name, bl.date_from, bl.date_to, bl.analytic_account_id, bl.purchase_budget_state
"""
