...
class ApprovalRequest(models.Model):
    ...
    sale_subscription_id = fields.Many2one('sale.subscription.agreement', string='Sale Subscription', required=False)
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.sale_subscription_id:
                    done_stage = self.env['sale.subscription.agreement.stage'].search([('is_refused','=',True)], limit=1)
                    self.sale_subscription_id.update({
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.sale_subscription_id:
                    done_stage = self.env['sale.subscription.agreement.stage'].search([('is_closed','=',True)], limit=1)
                    self.sale_subscription_id.update({
                        'stage_id': done_stage.id
                    })      
...
class ApprovalApprover(models.Model):
    ...
    sale_subscription_id = fields.Many2one(related='request_id.sale_subscription_id')