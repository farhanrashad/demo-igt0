...
class ReportPartnerLedgerExcel(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.write('H6', "Currency", format3)
        ...
        for o in result['docs']:
            ...
            sheet.write(row, col + 4, result['sum_partner'](result['data'], o, 'debit', data['currency_id']), format5)
            sheet.write(row, col + 5, result['sum_partner'](result['data'], o, 'credit', data['currency_id']), format5)
            sheet.write(row, col + 6, result['sum_partner'](result['data'], o, 'debit - credit', data['currency_id']),
                        format5)
            ...
            for line in result['lines'](result['data'], o, data['currency_id']):
                ...
                sheet.write(row, col + 7, str(currency_obj.name)+"("+str(data['currency_symbol'])+")", format7)
