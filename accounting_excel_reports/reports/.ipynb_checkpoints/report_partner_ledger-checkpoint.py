...
class ReportPartnerLedgerExcel(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.write('F6', "Account", format2)
        sheet.write('G6', "Ref", format2)
        sheet.write('H6', "Debit", format3)
        sheet.write('I6', "Credit", format3)
        sheet.write('J6', "Balance", format3)
        sheet.write('K6', "Currency", format3)
        ...
        for o in result['docs']:
            ...
            sheet.merge_range(row, col, row, col + 7, o.ref or '' + '-' + o.name, format4)
            ...
            sheet.write(row, col + 10, result['sum_partner'](result['data'], o, 'debit - credit', data['currency_id']),
                        format5)
            sheet.write(row, col + 7, str(currency_obj.name) + "(" + str(data['currency_symbol']) + ")", format5)
            row += 1
            for line in result['lines'](result['data'], o, data['currency_id']):
                ...
                sheet.write(row, col + 5, line['a_code'], format6)
                sheet.write(row, col + 6, line['displayed_name'] or '', format6)
                sheet.write(row, col + 7, (line['debit']), format7)
                sheet.write(row, col + 8, (line['credit']), format7)
                sheet.write(row, col + 9, (line['progress']), format7)
                sheet.write(row, col + 10, str(currency_obj.name)+"("+str(data['currency_symbol'])+")", format7)
